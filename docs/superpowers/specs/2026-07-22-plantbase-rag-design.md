...
packages/rag/src/
  ...
  lib/
    ...
    cohere-rerank.ts       # Cohere rerank-v3.5 (fetch)
...
- **Retrieval (query-idő, RO):** `retrieve.ts` — `raw` mód (query-embed → `similaritySearch` top-K) és `full` mód (HyDE → embed → `similaritySearch` top-N → Cohere rerank → top-K).
...
| **Rerank** | **Cohere** `rerank-v3.5` (cross-encoder) | Célépített query–dokumentum relevancia-pontozó. A rerank „tankönyvi" eszköze; a golden-set átrendezés-demóhoz stabil, magyarázható score-t ad — nem LLM-heurisztika. |
...
**Új env-kulcsok:** `OPENAI_API_KEY`, `COHERE_API_KEY` (az `ANTHROPIC_API_KEY` már megvan). **Új dep:** `@ai-sdk/openai`. A Cohere rerankot vékony `fetch`-kliens hívja (nincs nagy SDK-dep).
...
- **`full`**: `hyde(query)` → hipotetikus (angol) passzus → `embed(hyde)` → `similaritySearch(vec, topN=~20)` → **Cohere `rerank-v3.5`** a `query` ellen → top-K (~5). A rerank a HyDE-torzítást is korrigálja (a hipotetikus szöveg hozza a recall-t, a rerank adja a precíziót az *eredeti* kérdésre).
...
- **Dep:** `@ai-sdk/openai` (embedding), `pgvector/pgvector:pg17` docker image. Cohere rerank: `fetch` (nincs SDK-dep).
- **Env (`.env.example` bővül):** `OPENAI_API_KEY`, `COHERE_API_KEY`, opcionálisan `OPENAI_EMBED_MODEL`, `COHERE_RERANK_MODEL`, `ANTHROPIC_HYDE_MODEL`, `ANTHROPIC_ANSWER_MODEL`, `RAG_MIN_RERANK_SCORE`, `RAG_TOP_N`, `RAG_TOP_K`.
