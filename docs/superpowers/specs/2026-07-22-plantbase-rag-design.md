...
| **Rerank**                    | **Jina** `jina-reranker-v2-base-multilingual` (cross-encoder) | Célépített, többnyelvű query–dokumentum relevancia-pontozó. A rerank „tankönyvi" eszköze; a golden-set átrendezés-demóhoz stabil, magyarázható score-t ad — nem LLM-heurisztika. Ingyenes, azonnali self-serve kulccsal jár. |
