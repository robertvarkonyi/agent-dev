...
describe('assertSelectOnly', () => {
  it('elfogadja az egyszerű SELECT-et', () => {
    expect(assertSelectOnly('SELECT * FROM products')).toBe('SELECT * FROM products');
  });

  it('kis/nagybetűtől függetlenül elfogad', () => {
    expect(assertSelectOnly('select id from products')).toBe('select id from products');
  });

  it('elfogadja a WITH ... SELECT-et (CTE)', () => {
    const sql = 'WITH olcso AS (SELECT * FROM products WHERE price < 3000) SELECT name FROM olcso';
